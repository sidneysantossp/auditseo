...
const curitibaCity: CityConfig = {
  name: 'Curitiba',
  path: '/cidades/curitiba/'
};

const brasiliaCity: CityConfig = {
  name: 'Brasília',
...
function createHealthSeoPage(
  ...
  return {
    ...
    heroTitleSuffix: `EM ${city.name.toUpperCase()}`,
    heroSubtitle:
      `Em ${city.name}, a vertical de ${sector.summaryLabel} precisa alinhar cidade, especialidade e descoberta local para não depender só de indicação ou mídia paga. Quando a página comercial local é clara, o Google entende melhor quando aquela operação faz sentido.`,
    ...
    marketCards: [
      {
        ...
        copy: `Em ${city.name}, ${sector.marketFocus} costumam participar cedo da decisão. Sem página local forte, essa intenção se dispersa.`
      },
      {
    ...
    methodSteps: [
      {
        ...
        copy: `Mapeamos como a operação aparece hoje em ${city.name} e onde a arquitetura atual desperdiça intenção comercial local.`
      },
      {
        title: 'Papel da página da cidade',
        copy: 'Definimos a função da página local dentro da vertical setorial sem duplicar a página nacional.'
      },
      {
        title: 'Integração com Maps',
        copy: 'Alinhamos página, sinais locais e perfil da empresa para reforçar descoberta coerente.'
      },
      {
        title: 'Conversão',
        copy: 'Ajustamos copy, prova e CTA para transformar busca em contato com menos atrito.'
      },
      {
        title: 'Escala controlada',
        copy: 'Usamos desempenho real para decidir se a praça pede novas páginas derivadas ou reforço editorial.'
      }
    ],
    differentiators: [
    ...
    relatedLinks: createHealthRelatedLinks(
      ...
      localArticleLabel,
      gbpPath,
      `Google Meu Negócio para ${sector.displayTitle} em ${city.name}`
    ),
    faqItems: [
      {
        question: `${displayTitle} funciona em uma praça competitiva?`,
        answer: 'Funciona quando cidade, oferta, página comercial e mapa são tratados como um sistema e não como ativos isolados.'
      },
      {
        question: 'O que muda entre a página local e a página setorial?',
        answer: 'A página setorial organiza a tese comercial da vertical. A página local traduz essa tese para a intenção específica da cidade.'
      },
      {
        question: 'Google Meu Negócio entra junto nessa estratégia?',
        answer: 'Na maioria dos casos, sim. Perfil local e página comercial tendem a se reforçar mutuamente.'
      },
      {
        question: 'Vale abrir páginas ainda mais granulares dentro da cidade?',
        answer: 'Só quando houver demanda observável, função comercial clara e diferença real de contexto.'
      },
      {
        question: 'Quanto tempo leva para gerar resultado?',
        answer: 'Depende da base atual, da concorrência e da velocidade de execução. O foco correto é medir descoberta e contatos qualificados.'
      },
      {
        question: 'Essa página compete com a página setorial?',
        answer: 'Não deve competir. A vertical-mãe sustenta a tese comercial; a página local traduz essa tese para a cidade.'
      },
      {
        question: 'A operação precisa ter várias unidades para isso funcionar?',
        answer: 'Não. Mesmo operações mais enxutas podem depender fortemente da cidade e da descoberta local para gerar novos contatos.'
      },
      {
        question: `Quando contratar ${displayTitle.toLowerCase()} faz mais sentido?`,
        answer: `Quando ${city.name} já influencia a geração de demanda e a operação quer transformar melhor a busca local em contato.`
      }
    ],
...
function createHealthGbpPage(
  ...
  return {
    ...
    heroTitleSuffix: `EM ${city.name.toUpperCase()}`,
    heroSubtitle:
      `Em ${city.name}, o perfil local ajuda a filtrar descoberta, comparação e rota. Quando o Google Business Profile está alinhado à página comercial certa, a operação tende a ganhar mais relevância local e mais clareza de oferta.`,
    highlights: ['Google Maps', city.name, 'GBP', 'Busca por Proximidade'],
    heroCard: {
      title: 'Quando essa página faz sentido',
      copy:
        `Quando a operação já depende de mapa e comparação local em ${city.name}, mas ainda não integrou perfil, especialidade e página comercial em uma mesma lógica operacional.`,
      links: [
        { label: 'Ver Google Meu Negócio', path: '/servicos/google-meu-negocio/', reason: 'Serviço canônico principal.' },
        { label: `Ver ${sector.displayTitle.toLowerCase()} em ${city.name}`, path: seoLocalPath, reason: 'Página local complementar.' },
        { label: `Ver ${city.name}`, path: city.path, reason: 'Contexto geográfico principal.' }
      ]
    },
    stats: [
    ...
    marketCards: [
      {
        ...
        copy: `Em ${city.name}, parte relevante da decisão local passa por proximidade, percepção de confiança e facilidade de contato.`
      },
      {
        title: 'Perfil e página local',
        copy: 'Quando o perfil local está desalinhado com a página comercial, a descoberta perde força e a conversão tende a cair.'
      },
      {
        title: 'Especialidade e clareza',
        copy: 'A integração entre perfil, cidade e especialidade ajuda o Google e o usuário a entender melhor quando aquela operação faz sentido.'
      }
    ],
    ...
    methodSteps: [
      {
        ...
        copy: `Mapeamos como o perfil participa hoje da descoberta local em ${city.name} e onde perde relevância frente à concorrência.`
      },
      {
        title: 'Integração com o site',
        copy: 'Ajustamos a relação entre perfil local, página comercial da cidade e página setorial da vertical.'
      },
      {
        title: 'Sinais de relevância',
        copy: 'Organizamos os sinais visíveis da operação para reforçar geografia, contexto e especialidade.'
      },
      {
    ...
    proofPoints: [
      ...
      `Página local de ${sector.displayTitle.toLowerCase()} em ${city.name} já ativa.`,
    ...
    relatedLinks: [
      ...
      { label: `SEO Local para ${sector.displayTitle} em ${city.name}`, path: seoLocalPath, reason: 'Página local complementar.' },
    ...
    faqItems: [
      {
        question: `${displayTitle} sozinho resolve a descoberta local?`,
        answer: 'Não. O perfil ajuda muito, mas precisa estar alinhado a uma página local forte e à vertical principal da operação.'
      },
      {
        question: `Qual a diferença entre GBP e SEO local em ${city.name}?`,
        answer: 'GBP é parte da estratégia. SEO local inclui também página da cidade, estrutura do site e contexto comercial da operação.'
      },
      {
        question: 'A página do site realmente interfere no mapa?',
        answer: 'Sim. Quando cidade, especialidade e página comercial se reforçam, a descoberta local tende a ganhar mais coerência.'
      },
      {
        question: 'Quando essa frente merece prioridade?',
        answer: 'Quando a operação já depende de mapa para descoberta e quer aumentar a qualidade dos contatos vindos do Google.'
      },
      {
        question: 'Isso vale para uma única unidade?',
        answer: 'Sim. Mesmo operações mais enxutas podem depender bastante de proximidade e perfil local para serem encontradas.'
      },
      {
        question: 'Perfil desatualizado realmente prejudica?',
        answer: 'Prejudica porque reduz confiança, quebra coerência e deixa a operação mais frágil na comparação local.'
      },
      {
        question: 'Essa página compete com a página canônica de GBP?',
        answer: 'Não. A página canônica apresenta o serviço amplo; esta página traduz a oferta para a cidade e o contexto local.'
      },
      {
        question: `Quando contratar ${displayTitle.toLowerCase()} faz mais sentido?`,
        answer: `Quando a operação quer reforçar descoberta local em ${city.name}, integrar melhor mapa e site e melhorar a qualidade do contato vindo do Google.`
      }
    ],
...
function createLawCityPage(requireBrief: RequireBrief, city: CityConfig, slug: string): ServiceFirstCommercialPage {
  const displayTitle = `SEO para Escritórios de Advocacia em ${city.name}`;
  const localArticlePath = `/blog/seo-para-escritorios-de-advocacia-em-${city.name === 'Curitiba' ? 'curitiba' : 'brasilia'}-funciona/`;
  ...
  return {
    ...
    heroTitleSuffix: `EM ${city.name.toUpperCase()}`,
    heroSubtitle:
      `Em ${city.name}, o escritório precisa transformar autoridade institucional em descoberta orgânica mais previsível. Isso exige páginas por área de atuação, mensagem comercial clara e leitura local da praça.`,
    highlights: ['Advocacia', city.name, 'Áreas de Atuação', 'Busca Qualificada'],
    heroCard: {
      title: 'Quando essa página faz sentido',
      copy:
        `Quando o escritório já depende de indicação, quer ganhar visibilidade orgânica em áreas estratégicas e precisa estruturar melhor a presença comercial em ${city.name}.`,
      links: [
    ...
    marketCards: [
      {
        title: 'Busca por área de atuação',
        copy: 'O potencial cliente costuma pesquisar problema, área de atuação e contexto de contratação antes de preencher formulário.'
      },
      {
        title: 'Autoridade sem ruído',
        copy: 'A página precisa equilibrar credibilidade, clareza comercial e estrutura orientada à decisão sem exagero institucional.'
      },
      {
        title: 'Captação menos dependente de networking',
        copy: 'SEO entra para transformar busca qualificada em novas oportunidades sem depender só de relacionamento e indicação.'
      }
    ],
...
function createSoftwareCityPage(requireBrief: RequireBrief, city: CityConfig, slug: string): ServiceFirstCommercialPage {
  const displayTitle = `Consultoria SEO para Software B2B em ${city.name}`;
  const localArticlePath = `/blog/seo-para-software-b2b-em-${city.name === 'Curitiba' ? 'curitiba' : 'brasilia'}-como-funciona/`;
  ...
  return {
    ...
    heroTitleSuffix: `EM ${city.name.toUpperCase()}`,
    heroSubtitle:
      `Software B2B em ${city.name} exige uma arquitetura que capture busca por categoria, problema, comparação e solução. Não basta ter uma página institucional elegante se o produto não é encontrado nos momentos certos da jornada.`,
    ...
    marketCards: [
      {
        ...
        copy: 'O decisor costuma pesquisar categoria, contexto de uso e alternativas antes de falar com vendas. A arquitetura precisa refletir isso com precisão.'
      },
      {
        title: 'Venda consultiva',
        copy: 'A jornada é mais longa e menos impulsiva. O site precisa preparar comparação, confiança e narrativa comercial em múltiplas páginas.'
      },
      {
        title: 'Descoberta em IA e busca tradicional',
        copy: 'Para software B2B, descoberta orgânica e presença em respostas de IA tendem a se reforçar quando a entidade e a solução estão bem estruturadas.'
      }
    ],
...
export function buildCuritibaBrasiliaServiceFirstCommercialPages(requireBrief: RequireBrief): ServiceFirstCommercialPage[] {
  return [
    createHealthSeoPage(
      requireBrief,
      curitibaCity,
      odontologiaSector,
      '/servicos/seo-local-para-odontologia-em-curitiba/',
      'SEO Local para Odontologia em Curitiba',
      '/blog/seo-local-para-odontologia-em-curitiba-vale-a-pena/',
      'SEO local para odontologia em Curitiba vale a pena?',
      '/servicos/google-meu-negocio-para-odontologia-em-curitiba/'
    ),
    createHealthGbpPage(
      requireBrief,
      curitibaCity,
      odontologiaSector,
      '/servicos/google-meu-negocio-para-odontologia-em-curitiba/',
      'Google Meu Negócio para Odontologia em Curitiba',
      '/blog/seo-local-para-odontologia-em-curitiba-vale-a-pena/',
      'SEO local para odontologia em Curitiba vale a pena?',
      '/servicos/seo-local-para-odontologia-em-curitiba/'
    ),
    createHealthSeoPage(
      ...
      brasiliaCity,
      odontologiaSector,
      '/servicos/seo-local-para-odontologia-em-brasilia/',
      'SEO Local para Odontologia em Brasília',
      '/blog/seo-local-para-odontologia-em-brasilia-vale-a-pena/',
      'SEO local para odontologia em Brasília vale a pena?',
      '/servicos/google-meu-negocio-para-odontologia-em-brasilia/'
    ),
    createHealthGbpPage(
      requireBrief,
      brasiliaCity,
      odontologiaSector,
      '/servicos/google-meu-negocio-para-odontologia-em-brasilia/',
      'Google Meu Negócio para Odontologia em Brasília',
      '/blog/seo-local-para-odontologia-em-brasilia-vale-a-pena/',
      'SEO local para odontologia em Brasília vale a pena?',
      '/servicos/seo-local-para-odontologia-em-brasilia/'
    ),
    createHealthSeoPage(
      ...
      curitibaCity,
      dermatologiaSector,
      '/servicos/seo-local-para-dermatologistas-em-curitiba/',
      'SEO Local para Dermatologistas em Curitiba',
      '/blog/seo-local-para-dermatologistas-em-curitiba-vale-a-pena/',
      'SEO local para dermatologistas em Curitiba vale a pena?',
      '/servicos/google-meu-negocio-para-dermatologistas-em-curitiba/'
    ),
    createHealthGbpPage(
      requireBrief,
      curitibaCity,
      dermatologiaSector,
      '/servicos/google-meu-negocio-para-dermatologistas-em-curitiba/',
      'Google Meu Negócio para Dermatologistas em Curitiba',
      '/blog/seo-local-para-dermatologistas-em-curitiba-vale-a-pena/',
      'SEO local para dermatologistas em Curitiba vale a pena?',
      '/servicos/seo-local-para-dermatologistas-em-curitiba/'
    ),
    createHealthSeoPage(
      ...
      brasiliaCity,
      dermatologiaSector,
      '/servicos/seo-local-para-dermatologistas-em-brasilia/',
      'SEO Local para Dermatologistas em Brasília',
      '/blog/seo-local-para-dermatologistas-em-brasilia-vale-a-pena/',
      'SEO local para dermatologistas em Brasília vale a pena?',
      '/servicos/google-meu-negocio-para-dermatologistas-em-brasilia/'
    ),
    createHealthGbpPage(
      requireBrief,
      brasiliaCity,
      dermatologiaSector,
      '/servicos/google-meu-negocio-para-dermatologistas-em-brasilia/',
      'Google Meu Negócio para Dermatologistas em Brasília',
      '/blog/seo-local-para-dermatologistas-em-brasilia-vale-a-pena/',
      'SEO local para dermatologistas em Brasília vale a pena?',
      '/servicos/seo-local-para-dermatologistas-em-brasilia/'
    ),
    createLawCityPage(requireBrief, curitibaCity, '/servicos/seo-para-escritorios-de-advocacia-em-curitiba/'),
    createLawCityPage(requireBrief, brasiliaCity, '/servicos/seo-para-escritorios-de-advocacia-em-brasilia/'),
    createSoftwareCityPage(requireBrief, curitibaCity, '/servicos/consultoria-seo-para-software-b2b-em-curitiba/'),
    createSoftwareCityPage(requireBrief, brasiliaCity, '/servicos/consultoria-seo-para-software-b2b-em-brasilia/')
  ];
}
